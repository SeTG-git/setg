...
def ana_json(json_data):
    ...
    activity_sources = {activity['name']: []
    ...
    for transition in json_data['transitions']:
        if transition['dest'] not in activity_sources or transition['scr'] not in activity_sources:
            continue
        activity_sources[transition['dest']].append(
            transition['scr'] for activity in json_data['activities'])
...
def gen_all():
    ...
    for k in ana_scam_j:
        ...
        save_to_file(
            ana_res, sp='/home/aibot/workspace/SquiDroidAgent/data/setg', fn=f'setg_{k}.csv')
        logger.debug(f'save setg_{k} to setg succ.')


def gen_single():
    p = "./data/1b76d42256ec8e9ce6b699aeef85fbbb.json"
    k = "1b76d42256ec8e9ce6b699aeef85fbbb"
    with open(p, mode="r", encoding="utf-8") as f:
        json_data = json.load(f)
    ana_res = ana_json(json_data)
    save_to_file(
...
if __name__ == "__main__":
    save_to_file({'k': 'v', 'k2': 'v2', 'k3': 'v3'},
                 sp='/home/aibot/workspace/SquiDroidAgent/data/setg', fn=f'setg_test.csv')
    main()
