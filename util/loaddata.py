...
def load_rawdata(name, mode = 'train'):
    if name == 'streamspot':
        path = './data/streamspot'
        ...
        if os.path.exists(path+fn ):
            print('Loading processed {} dataset ...'.format(name))
            raw_data = pkl.load(open(path+fn , 'rb'))
        else:
            print("Loading dataset {} failed...".format(name))
    return raw_data


def load_batch_level_dataset(dataset_name, mode='train'):
    dataset = load_rawdata(dataset_name,mode = mode)
    graph, _ , _ = dataset[0]
    node_feature_dim = 0
    node_feature_dim = 2827
    for g, _, _ in dataset:
        node_feature_dim = max([node_feature_dim, g.ndata["type"].max().item(), 2827])
        node_feature_dim = min(node_feature_dim, 27109)
    edge_feature_dim = 0
    for g, _, _ in dataset:
        edge_feature_dim = max(edge_feature_dim, g.edata["type"].max().item())
        edge_feature_dim = min(27109, edge_feature_dim)
    node_feature_dim += 1
    edge_feature_dim += 1
    full_dataset = [i for i in range(len(dataset))]
    # train_dataset = [i for i in range(len(dataset)) if dataset[i][1] == 0]
    train_dataset = [i for i in range(len(dataset))]  # 全部数据都是训练数据
