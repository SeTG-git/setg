...
from llm_connector.colorful import print亮红, print亮绿, print亮蓝
...
def read_single_conf_with_lru_cache(arg):
    ...
    try:
        # 优先级1. 获取环境变量作为配置
        default_ref = getattr(importlib.import_module('llm_connector.config'), arg) # 读取默认值作为数据类型转换的参考
        r = read_env_variable(arg, default_ref)
    except:
        try:
            # 优先级2. 获取config_private中的配置
            r = getattr(importlib.import_module('llm_connector.config_private'), arg)
        except:
            # 优先级3. 获取config中的配置
            r = getattr(importlib.import_module('llm_connector.config'), arg)
...
def set_conf(key, value):
    from data.llm_connector.toolbox.toolbox import read_single_conf_with_lru_cache
