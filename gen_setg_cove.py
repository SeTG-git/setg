...
def analyze_activity(activity, src_activities_desc=""):
    ...
    analysis_verified = analysis_result = request_gpt_model_in_new_thread(
        verfiy_prompt,
        llm_kwargs,
        chatbot,
        history=[analysis_prompt, analysis_result],
        sys_prompt="你是一位经验丰富的移动应用程序分析师，专门分析应用程序结构和流程。",
        key_index=0
    )
    ...
    analysis_result2 = request_gpt_model_in_new_thread(
        ...
        llm_kwargs,
        chatbot,
        history=[analysis_verified],
        sys_prompt="你是一位经验丰富的阅读理解大师，你阅读上面activity描述，用**一句话**概括activity功能. 例如：“splashActivity页面功能是启动这个健康APP”",
        key_index=0
    )
...
def save_to_file(ana_res, sp='/home/aibot/workspace/SquiDroidAgent/data/setg_cove', fn='setg_default.csv'):
...
def main():
    ...
    generated = os.listdir('/home/aibot/workspace/SquiDroidAgent/data/setg_cove')
    for k in ana_scam_j:
        ...
        for i in range(5):
            ana_res = ana_json(json_data)
            save_to_file(ana_res, sp='/home/aibot/workspace/SquiDroidAgent/data/setg_cove', fn=f'setg_{k}_{i}.csv')
            logger.debug(f'save setg_{k}_{i} to setg succ.')

        save_to_file(ana_res, sp='/home/aibot/workspace/SquiDroidAgent/data/setg_cove', fn=f'setg_{k}.csv')
        logger.debug(f'save setg_{k} to setg succ.')

if __name__ == "__main__":
    save_to_file({'k':'v','k2':'v2','k3':'v3'}, sp='/home/aibot/workspace/SquiDroidAgent/data/setg_cove', fn=f'setg_cove.csv')
    main()
